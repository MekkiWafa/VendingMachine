...
public class UserIOConsoleImpl implements UserIO {
    ...
    public BigDecimal readBigDecimal(String msgPrompt) {
        while (true) {
            try {
                BigDecimal aBigDecimal = new BigDecimal(readString(msgPrompt));
                return aBigDecimal.setScale(2, RoundingMode.HALF_UP);

                //  return BigDecimal.valueOf(readDouble(msgPrompt));
            } catch (NumberFormatException e) {
                this.print("Input error. Please try again.");
            }
        }
